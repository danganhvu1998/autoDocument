@extends('layouts.app')

@section('content')
    <div class="row">
        <div class="col-md-8">
            <h1>View <strong style="color:brown"> {{$document->document_name}} </strong></h1>
        </div>
        <div class="col-md-2">
            <a href="/documents" class="btn btn-primary">All Documents</a>
        </div>
        <div class="col-md-2">
            <a href="/documents/edit/{{$document->id}}" class="btn btn-primary">Edit Document</a>
        </div>
    </div>

    <h3>{{$document->document_name}} is having ...</h3>
    <ul>
        @foreach($hasDefines as $hasDefine)
            <li><strong style="color:green">{{$hasDefine->name}}<br></strong></li>
        @endforeach
    </ul>
    <hr>
    
    <h3>{{$document->document_name}} is <strong>NOT</strong> having ...</h3>
    <ul>
        @foreach($notHasDefines as $notHasDefine)
            <li><strong style="color:red">{{$notHasDefine->name}}<br></strong></li>
        @endforeach
    </ul>
    <hr>
    
@endsection
